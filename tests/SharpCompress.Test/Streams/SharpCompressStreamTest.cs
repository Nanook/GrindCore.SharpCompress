using System;
using System.Buffers;
using System.IO;
using System.Text;
using SharpCompress.Compressors.LZMA;
using SharpCompress.IO;
using Xunit;
using ZstdSharp.Unsafe;
using System.Linq;

namespace SharpCompress.Test.Streams;

public class SharpCompressStreamTests
{
    private static void createData(MemoryStream ms)
    {
        using (BinaryWriter bw = new BinaryWriter(ms, Encoding.UTF8, true))
        {
            //write offset every 4 bytes - easy to test position
            for (int i = 0; i < ms.Length; i += 4)
            {
                bw.Write(i);
                var buffer = ArrayPool<byte>.Shared.Rent(1024);
            }
        }
        ms.Position = 0;
    }

    [Fact]
    public void BufferReadTest()
    {
        byte[] data = ArrayPool<byte>.Shared.Rent(0x100000);
        byte[] test = ArrayPool<byte>.Shared.Rent(0x1000);
        using (MemoryStream ms = new MemoryStream(data))
        {
            createData(ms);

            using (SharpCompressStream scs = new SharpCompressStream(ms, true, false, 0x10000))
            {
                IStreamStack stack = (IStreamStack)scs;

                scs.Seek(0x1000, SeekOrigin.Begin);
                Assert.Equal(0x1000, scs.Position); //position in the SharpCompressionStream (with 0xf000 remaining in the buffer)
                Assert.Equal(0x1000, ms.Position); //initial seek + full buffer read

                scs.Read(test, 0, test.Length); //read bytes 0x1000 to 0x2000
                Assert.Equal(0x2000, scs.Position); //stream has correct position
                Assert.True(data.Skip(test.Length).Take(test.Length).SequenceEqual(test)); //is the data correct
                Assert.Equal(0x11000, ms.Position); //seek plus read bytes

                scs.Seek(0x500, SeekOrigin.Begin); //seek before the buffer start
                scs.Read(test, 0, test.Length); //read bytes 0x500 to 0x1500
                Assert.Equal(0x1500, scs.Position); //stream has correct position
                Assert.True(data.Skip(0x500).Take(test.Length).SequenceEqual(test)); //is the data correct
                Assert.Equal(0x10500, ms.Position); //seek plus read bytes
            }
        }

        ArrayPool<byte>.Shared.Return(data);
        ArrayPool<byte>.Shared.Return(test);
    }

    [Fact]
    public void BufferReadAndSeekTest()
    {
        byte[] data = ArrayPool<byte>.Shared.Rent(0x100000);
        byte[] test = ArrayPool<byte>.Shared.Rent(0x1000);
        using (MemoryStream ms = new MemoryStream(data))
        {
            createData(ms);

            using (SharpCompressStream scs = new SharpCompressStream(ms, true, false, 0x10000))
            {
                IStreamStack stack = (IStreamStack)scs;

                scs.Read(test, 0, test.Length); //read bytes 0 to 0x1000
                Assert.True(data.Take(test.Length).SequenceEqual(test)); //is the data correct
                Assert.Equal(0x1000, scs.Position); //stream has correct position
                Assert.Equal(0x10000, ms.Position); //moved the base stream on by the size of the buffer not what was requested

                scs.Read(test, 0, test.Length); //read bytes 0x1000 to 0x2000
                Assert.Equal(0x2000, scs.Position); //stream has correct position
                Assert.True(data.Skip(test.Length).Take(test.Length).SequenceEqual(test)); //is the data correct
                Assert.Equal(0x10000, ms.Position); //the base stream has not moved

                //rewind the buffer
                stack.Rewind(0x1000); //rewind buffer back by 0x1000 bytes

                //repeat the previous test
                scs.Read(test, 0, test.Length); //read bytes 0x1000 to 0x2000
                Assert.Equal(0x2000, scs.Position); //stream has correct position
                Assert.True(data.Skip(test.Length).Take(test.Length).SequenceEqual(test)); //is the data correct
                Assert.Equal(0x10000, ms.Position); //the base stream has not moved

            }
        }

        ArrayPool<byte>.Shared.Return(data);
        ArrayPool<byte>.Shared.Return(test);
    }

    [Fact]
    public void RewindOnlyOuterMostBufferStream()
    {
        byte[] data = ArrayPool<byte>.Shared.Rent(0x10000);
        byte[] test = ArrayPool<byte>.Shared.Rent(0x100);
        using (MemoryStream ms = new MemoryStream(data))
        {
            createData(ms);

            // Create an inner SharpCompressStream with a buffer
            using (var inner = new SharpCompressStream(ms, leaveOpen: true, throwOnDispose: false, bufferSize: 0x1000))
            // Wrap it in an outer SharpCompressStream with a buffer
            using (var outer = new SharpCompressStream(inner, leaveOpen: true, throwOnDispose: false, bufferSize: 0x800, forceBuffer: true))
            {
                var stack = (IStreamStack)outer;

                Assert.Equal(0x1000, ((IStreamStack)inner).BufferSize);
                Assert.Equal(0x800, ((IStreamStack)outer).BufferSize);

                // Read some data through the stack
                int bytesRead = outer.Read(test, 0, test.Length);
                Assert.Equal(test.Length, bytesRead);

                // Save positions before rewind
                long outerPosBefore = outer.Position;
                long innerPosBefore = inner.Position;

                // Rewind the stack (should only affect the outer stream's buffer)
                stack.Rewind(bytesRead);

                // After rewind, outer's position should be reset, inner's should be unchanged
                Assert.Equal(0, outer.Position);
                Assert.Equal(innerPosBefore, inner.Position);

                // Read again after rewind, should get the same data as before
                byte[] test2 = new byte[test.Length];
                int bytesRead2 = outer.Read(test2, 0, test2.Length);
                Assert.Equal(test.Length, bytesRead2);
                Assert.True(test.SequenceEqual(test2));
            }
        }
        ArrayPool<byte>.Shared.Return(data);
        ArrayPool<byte>.Shared.Return(test);
    }

    [Fact]
    public void DontCreateDuplicate()
    {
        // Create an inner SharpCompressStream with a buffer
        using (var inner = new SharpCompressStream(Stream.Null, leaveOpen: true, throwOnDispose: false, bufferSize: 0x1000))
        // Wrap it in an outer SharpCompressStream with a buffer
        using (var outer = SharpCompressStream.Create(inner, leaveOpen: true, throwOnDispose: false, bufferSize: 0x1000))
        {
            Assert.Equal(inner, outer);
        }
    }

    [Fact]
    public void DuplicateNoBuffer()
    {
        // Create an inner SharpCompressStream with a buffer
        using (var inner = new SharpCompressStream(Stream.Null, leaveOpen: true, throwOnDispose: false, bufferSize: 0x1000))
        // Wrap it in an outer SharpCompressStream with a buffer
        using (var outer = new SharpCompressStream(inner, leaveOpen: true, throwOnDispose: false, bufferSize: 0x1000))
        {
            Assert.Equal(0x1000, ((IStreamStack)inner).BufferSize);
            Assert.Equal(0x0, ((IStreamStack)outer).BufferSize);
        }
    }
}
