...
#pragma warning restore CA1307 // Specify StringComparison for clarity
    ...
    private async ValueTask EmitHeaderAsync(CancellationToken cancellationToken = default)
    {
        var commentBytes = (_comment is null) ? null : _encoding.GetBytes(_comment);
        var filenameBytes = (_fileName is null) ? null : _encoding.GetBytes(_fileName);
    ...
    private async ValueTask InitializeCompressModeAsync(
    ...
    )
    {
        ...
        {
            _customHeaderMode = true;
            await EmitHeaderAsync(cancellationToken).ConfigureAwait(false);
            _crc32 = new CRC32();
            _originalSize = 0;
    ...
    private async ValueTask WriteGzipTrailerAsync(CancellationToken cancellationToken = default)
    {
        var trailer = new byte[8];
        BinaryPrimitives.WriteUInt32LittleEndian(trailer.AsSpan(0), (uint)_crc32!.Crc32Result);
        BinaryPrimitives.WriteUInt32LittleEndian(trailer.AsSpan(4), (uint)_originalSize);
        await _inputStream
            .WriteAsync(trailer, 0, trailer.Length, cancellationToken)
            .ConfigureAwait(false);
    }
    ...
    )
    {
        ...
        await InitializeCompressModeAsync(cancellationToken).ConfigureAwait(false);

        if (_customHeaderMode)
        {
            _crc32!.SlurpBlock(buffer, offset, count);
            _originalSize += count;
            await _rawDeflateStream!
    ...
    )
    {
        ...
        if (_customHeaderMode)
        {
            ...
            await _rawDeflateStream!.WriteAsync(buffer, cancellationToken).ConfigureAwait(false);
            return;
        }
    ...
    public override async ValueTask DisposeAsync()
    {
        ...
        try
        {
            if (_customHeaderMode && _rawDeflateStream != null)
            {
                // Complete the raw deflate stream asynchronously (flushes via async path,
                // sets _complete = true so subsequent Dispose won't call sync Flush).
                await _rawDeflateStream.CompleteAsync().ConfigureAwait(false);
                _rawDeflateStream.Dispose();
                await WriteGzipTrailerAsync().ConfigureAwait(false);
            }
            else if (_grindCoreStream != null)
            {
                // Complete the GrindCore GZip stream asynchronously before sync Dispose.
