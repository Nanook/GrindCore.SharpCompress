...
public class SevenZipZstdLevel19WriteExtractTests : ArchiveTests
{
    ...
    public void SevenZip_Zstd_Level19_Write_Then_Extract_VerifyLengths(int size)
    {
        ...
        try
        {
            using (var writer = WriterFactory.OpenWriter(mem, ArchiveType.SevenZip, writerOptions))
            {
        ...
        mem.Position = 0;
        using var archive = SevenZipArchive.OpenArchive(mem);
        var entry = archive.Entries.Single(e => !e.IsDirectory);

        if (size == 0)
        {
            // Empty files have no folder/coder in 7z format
