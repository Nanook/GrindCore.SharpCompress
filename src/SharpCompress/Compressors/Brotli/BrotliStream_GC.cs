...
/// <summary>
/// Lightweight wrapper for GrindCore BrotliStream that implements SharpCompress interfaces
/// </summary>
public sealed class BrotliStream : Stream, IStreamStack
{
    ...
    /// <param name="compressionLevel">The compression level (1-11)</param>
    /// <param name="leaveOpen">Whether to leave the base stream open when disposing</param>
    public BrotliStream(Stream baseStream, int compressionLevel, bool leaveOpen = false)
    {
        _grindCoreStream = new GrindCoreBrotliStream(
            baseStream,
    ...
    /// <summary>
    /// Initializes a new instance of the BrotliStream class for decompression
    /// </summary>
    /// <param name="baseStream">The stream to read compressed data from</param>
    /// <param name="leaveOpen">Whether to leave the base stream open when disposing</param>
    public BrotliStream(Stream baseStream, bool leaveOpen = false)
    {
        _grindCoreStream = new GrindCoreBrotliStream(
            baseStream,
            new CompressionOptions
    ...
    /// <summary>
    /// Initializes a new instance of the BrotliStream class with compression mode
    /// </summary>
    ...
    )
    {
        ...
        _grindCoreStream = new GrindCoreBrotliStream(baseStream, options);
        _leaveOpen = leaveOpen;
    }
#else
    /// <summary>
    /// Initializes a new instance of the BrotliStream class (fallback when GrindCore is not available)
    /// </summary>
    public BrotliStream(
        Stream baseStream,
        CompressionMode mode,
    ...
    {
        throw new NotSupportedException("Brotli compression requires GrindCore library");
    }
    ...
    {
        // Brotli streams typically don't support seeking, so this is a no-op
    }
