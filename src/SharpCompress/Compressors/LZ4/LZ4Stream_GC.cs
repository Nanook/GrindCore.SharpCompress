...
using GrindCoreLz4Stream = Nanook.GrindCore.Lz4.Lz4Stream;
#endif

namespace SharpCompress.Compressors.LZ4;

/// <summary>
/// Lightweight wrapper for GrindCore LZ4Stream that implements SharpCompress interfaces
/// </summary>
public sealed partial class LZ4Stream : Stream, IStreamStack
{
#if GRINDCORE
    ...
    /// <param name="compressionLevel">The compression level (1-12)</param>
    /// <param name="leaveOpen">Whether to leave the base stream open when disposing</param>
    public LZ4Stream(Stream baseStream, int compressionLevel, bool leaveOpen = false)
    {
        ...
        _grindCoreStream = new GrindCoreLz4Stream(baseStream, options);
        _leaveOpen = leaveOpen;
    }

    /// <summary>
    /// Initializes a new instance of the LZ4Stream class for decompression
    /// </summary>
    /// <param name="baseStream">The stream to read compressed data from</param>
    /// <param name="leaveOpen">Whether to leave the base stream open when disposing</param>
    public LZ4Stream(Stream baseStream, bool leaveOpen = false)
    {
        var options = new CompressionOptions
        {
    ...
    /// <summary>
    /// Initializes a new instance of the LZ4Stream class with compression mode
    /// </summary>
...
#else
    /// <summary>
    /// Initializes a new instance of the LZ4Stream class (fallback when GrindCore is not available)
    /// </summary>
    public LZ4Stream(
        Stream baseStream,
        CompressionMode mode,
    ...
    {
        // LZ4 streams typically don't support seeking, so this is a no-op
    }
