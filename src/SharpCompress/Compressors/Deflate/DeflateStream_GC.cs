...
public class DeflateStream : Stream, IStreamStack
{
    ...
    )
    {
        ...
        if (!_isEncoder)
        {
            var options = new NGC.CompressionOptions()
            {
                ...
                BufferSize = 0x10000,
                LeaveOpen = true, // Always leave open - we manage base stream disposal ourselves
                Version = isNg
            ...
            GrindCoreBufferHelper.ConfigureAsyncOnlyIfNeeded(options, _baseStream);
    ...
    /// <summary>
    /// Backwards-compatible InputBuffer property that maps to InternalBuffer.
    /// </summary>
    public MemoryStream InputBuffer => InternalBuffer;
    ...
    public override async Task FlushAsync(CancellationToken cancellationToken)
    {
        ...
        // Mirror synchronous Flush() behavior so async callers also rewind over-read bytes.
        if (!_modeInitialized)
        {
        ...
        if (_readToCompressMode)
        {
            // Ensure any pending data is compressed
            EnsureCompressionComplete();
            return;
        }
        ...
        if (!_isEncoder)
        {
            // Decompression: flush underlying grindcore stream and rewind unused input bytes
            if (_grindCoreStream != null)
            {
...
#pragma warning restore VSTHRD103
    ...
    public override async ValueTask DisposeAsync()
    {
        ...
        try
        {
            ...
            {
                // Finalize compression in read-to-compress mode using async path
                if (_grindCoreStream != null)
                {
            ...
            {
                // Complete the GrindCore stream asynchronously before sync Dispose.
                // CompleteAsync uses onFlushAsync (which calls BaseStream.FlushAsync),
                // and sets _complete = true so subsequent Dispose() won't call sync Flush.
                await _grindCoreStream.CompleteAsync().ConfigureAwait(false);
            }

            // Dispose GrindCore stream (which now has LeaveOpen=true, so won't dispose base stream).
            // Since CompleteAsync already flushed and set _complete, onDispose→onFlush is a no-op.
            _grindCoreStream?.Dispose();
